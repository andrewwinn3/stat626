...
umcsent <- read_csv(file = '~/STAT626/stat626/Data/UMCSENT2.csv') # Change path
...
comp = read.csv("~/STAT626/stat626/Data/UMCSENT_COMPONENTS.csv") # Change Path
comp$DATE = as.Date(paste(comp$Year, comp$Month, "01", sep="-"))
```

# Investigation of differenced series
...
source('~/STAT626/stat626/ARMAroots_RFunctions.R') # Change path
# ACF AND PACF PLOTS
...
types = c("none", "none", "none", "none", "none", "none", "none")
df0 = c(1,1,1,1,1,1)
...
#The AIC picked ARIMA(5,1,0) and the BIC picked ARIMA(4,1,0). Since the lowest AIC is ARIMA(5,1,0), we will choose that model for PFE. 
...
We now repeat the same processes as before using second-order difference series.

```{r}
# A data frame of all second-order difference series
DIFF2 = data.frame(
      UMCSENT = c(NA,NA, diff(umcsent.df$UMCSENT, 2)),
      PFC = c(NA,NA, diff(comp$Personal.Finance.Current, 2)),
      PFE = c(NA,NA, diff(comp$Personal.Finance.Expected, 2)),
      BC12 = c(NA,NA, diff(comp$Business.Condition.12.Months, 2)),
      BC5 = c(NA,NA, diff(comp$Business.Condition.5.Years, 2)),
      BC = c(NA,NA, diff(comp$Buying.Conditions, 2))
)
```

```{r}
# ACF AND PACF PLOTS
ggtsdisplay(DIFF2$UMCSENT, xlab = "Time", ylab = "index 1966:Q1 = 100", main= "Second-Order Difference Series of Consumer Sentiment")
ggtsdisplay(DIFF2$PFC, xlab = "Time", ylab = "index 1966:Q1 = 100", main= "Second-Order Difference Series of Current Personal Finance")
ggtsdisplay(DIFF2$PFE, xlab = "Time", ylab = "index 1966:Q1 = 100", main= "Second-Order Difference Series of Expected Personal Finance")
ggtsdisplay(DIFF2$BC12, xlab = "Time", ylab = "index 1966:Q1 = 100", main= "Second-Order Difference Series of Business conditions in 12 months")
ggtsdisplay(DIFF2$BC5, xlab = "Time", ylab = "index 1966:Q1 = 100", main= "Second-Order Difference Series of Business Conditions in 5 Years")
ggtsdisplay(DIFF2$BC, xlab = "Time", ylab = "index 1966:Q1 = 100", main= "Second-Order Difference Series of Buying Conditions")
```
...
DIFF2.dADF = matrix(ncol = 7, nrow = k)
types = c("none", "none", "none", "none", "none", "none", "none")
df0 = c(1,1,1,1,1,1)

colnames(DIFF2.dADF) = c("cri value", "adf-aic", "lag-aic", "signif-aic", "adf-bic", "lag-bic", "sign-bic")
rownames(DIFF2.dADF) = names(DIFF2)

for(i in 1:k){
  obj = ur.df(DIFF2[-(1:2),i], type = types[i], lag = p.max, selectlags = "AIC");
  DIFF2.dADF[i,1] = obj@cval[1,2]
  DIFF2.dADF[i,2] = obj@teststat[1]
  DIFF2.dADF[i,3] = obj@testreg$df[1] - df0[i]
  DIFF2.dADF[i,4] = DIFF2.dADF[i,2] < DIFF2.dADF[i,1] #1: Reject Ho
  obj = ur.df(DIFF2[-(1:2),i], type = types[i], lag = p.max, selectlags = "BIC");
  DIFF2.dADF[i,5] = obj@teststat[1]
  DIFF2.dADF[i,6] = obj@testreg$df[1] - df0[i]
  DIFF2.dADF[i,7] = DIFF2.dADF[i,5] < DIFF2.dADF[i,1] #1: Reject Ho
...
DIFF2.fit1aic = auto.arima(DIFF2[,1], d = 1, seasonal = F, ic = "aic")
DIFF2.DF1 = round(log(length(DIFF2[,1])))
DIFF2.fit2aic = auto.arima(DIFF2[,2], d = 1, seasonal = F, ic = "aic")
DIFF2.DF2 = round(log(length(DIFF2[,2])))
DIFF2.fit3aic = auto.arima(DIFF2[,3], d = 1, seasonal = F, ic = "aic")
DIFF2.DF3 = round(log(length(DIFF2[,3])))
DIFF2.fit4aic = auto.arima(DIFF2[,4], d = 1, seasonal = F, ic = "aic")
DIFF2.DF4 = round(log(length(DIFF2[,4])))
DIFF2.fit5aic = auto.arima(DIFF2[,5], d = 1, seasonal = F, ic = "aic")
DIFF2.DF5 = round(log(length(DIFF2[,5])))
DIFF2.fit6aic = auto.arima(DIFF2[,6], d = 1, seasonal = F, ic = "aic")
DIFF2.DF6 = round(log(length(DIFF2[,6])))

#fitting the TS with BIC

DIFF2.fit1bic = auto.arima(DIFF2[,1], d = 1, seasonal = F, ic = "bic")
DIFF2.fit2bic = auto.arima(DIFF2[,2], d = 1, seasonal = F, ic = "bic")
DIFF2.fit3bic = auto.arima(DIFF2[,3], d = 1, seasonal = F, ic = "bic")
DIFF2.fit4bic = auto.arima(DIFF2[,4], d = 1, seasonal = F, ic = "bic")
DIFF2.fit5bic = auto.arima(DIFF2[,5], d = 1, seasonal = F, ic = "bic")
DIFF2.fit6bic = auto.arima(DIFF2[,6], d = 1, seasonal = F, ic = "bic")
```
...
cat("Portmanteau tests of DIFF2.fitted residuals")
checkresiduals(DIFF2.fit1aic, lag = DIFF2.DF1 + length(DIFF2.fit1aic$model$phi))
checkresiduals(DIFF2.fit2aic, lag = DIFF2.DF2 + length(DIFF2.fit2aic$model$phi))
checkresiduals(DIFF2.fit3aic, lag = DIFF2.DF3 + length(DIFF2.fit3aic$model$phi))
checkresiduals(DIFF2.fit4aic, lag = DIFF2.DF4 + length(DIFF2.fit4aic$model$phi))
checkresiduals(DIFF2.fit5aic, lag = DIFF2.DF5 + length(DIFF2.fit5aic$model$phi))
checkresiduals(DIFF2.fit6aic, lag = DIFF2.DF6 + length(DIFF2.fit6aic$model$phi))
```
